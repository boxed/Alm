...
/// A local variable: an SSA value, or a stack slot loaded on each use
/// (tail-loop parameters).
#[derive(Clone, Copy)]
enum Binding<'ctx> {
...
impl<'ctx> Codegen<'ctx> {
    ...
    fn declare_runtime(&mut self) {
        ...
        let table: &[(&'static str, Ret, &[Ty])] = &[
            ...
            ("rt_chr", Ptr, &[I32]),
            ("rt_str", Ptr, &[P, I64]),
            ("rt_ctor", Ptr, &[P, I32, I32, P]),
            ("rt_list", Ptr, &[I32, P]),
            ("rt_tuple", Ptr, &[I32, P]),
            ("rt_closure", Ptr, &[P, I32, I32, P]),
            ("rt_closure_set", Void, &[P, I32, P]),
            ("rt_apply", Ptr, &[P, I32, P]),
            ("rt_record_new", Ptr, &[I32]),
            ("rt_record_set", Void, &[P, I32, P, P]),
            ("rt_record_clone", Ptr, &[P]),
            ("rt_record_replace", Void, &[P, P, P]),
            ("rt_access", Ptr, &[P, P]),
            ("rt_ctor_arg", Ptr, &[P, I32]),
            ("rt_tuple_item", Ptr, &[P, I32]),
            ("rt_list_head", Ptr, &[P]),
            ("rt_list_tail", Ptr, &[P]),
            ("rt_is_true", Bool, &[P]),
            ("rt_is_ctor", Bool, &[P, I32]),
            ("rt_is_bool", Bool, &[P, I1]),
            ("rt_is_int", Bool, &[P, I64]),
            ("rt_is_chr", Bool, &[P, I32]),
            ("rt_is_str", Bool, &[P, P, I64]),
            ("rt_is_cons", Bool, &[P]),
            ("rt_is_nil", Bool, &[P]),
            ("rt_add", Ptr, &[P, P]),
            ("rt_sub", Ptr, &[P, P]),
        ...
        for (name, ret, params) in table {
            ...
            let fn_type = match ret {
                Ret::Ptr => self.ptr_t.fn_type(&param_types, false),
        ...
        // Singleton values loaded directly by generated code.
        for name in ["rt_true_v", "rt_false_v", "rt_unit_v"] {
            let g = self.module.add_global(self.ptr_t, None, name);
    ...
    fn emit_program(&mut self, program: &Program) {
        ...
        for f in &program.functions {
            let params = vec![self.ptr_t.into(); f.params.len()];
            let fn_type = self.ptr_t.fn_type(&params, false);
            let fv = self
                .module
        ...
        for value in &program.values {
            let g = self.module.add_global(self.ptr_t, None, &value.name);
            g.set_initializer(&self.ptr_t.const_null());
    ...
    /// `alm_main` returns the entry module's `main`, or null.
    fn emit_main(&mut self, main: Option<&str>) {
        ...
        let result = match main {
            Some(name) => self
                .builder
                .build_load(self.ptr_t, self.globals[name].as_pointer_value(), "main")
                .unwrap()
                .into_pointer_value(),
    ...
    fn emit_function(&mut self, function: &Function) {
        ...
        let params: Vec<PointerValue> = (0..function.params.len())
            .map(|i| fv.get_nth_param(i as u32).unwrap().into_pointer_value())
            .collect();
    ...
    fn emit_body(
        ...
        body: &Expr,
        params: &[(String, PointerValue<'ctx>)],
        captures: usize,
        tail_recursive: bool,
    ) {
        self.cur_fn = Some(fv);
        ...
        for (i, (name, value)) in params.iter().enumerate() {
            if !tail_recursive || i < captures {
                ...
                let slot = self.entry_builder.build_alloca(self.ptr_t, name).unwrap();
    ...
    fn alloca(&self, name: &str) -> PointerValue<'ctx> {
        self.entry_builder.build_alloca(self.ptr_t, name).unwrap()
    }

    fn call_ptr(&self, name: &str, args: &[BasicMetadataValueEnum<'ctx>]) -> PointerValue<'ctx> {
        self.builder
    ...
    /// A pointer to a heap array of the given values (or null when empty),
    /// for the runtime's variadic-by-array entry points.
    fn value_array(&self, values: &[PointerValue<'ctx>]) -> PointerValue<'ctx> {
        if values.is_empty() {
            return self.ptr_t.const_null();
        }
        let array_type = self.ptr_t.array_type(values.len() as u32);
    ...
    fn foreign(&mut self, symbol: &str) -> PointerValue<'ctx> {
        let global = *self
            .externs
            .entry(symbol.to_string())
            .or_insert_with(|| self.module.add_global(self.ptr_t, None, symbol));
        self.builder
            .build_load(self.ptr_t, global.as_pointer_value(), "f")
            .unwrap()
            .into_pointer_value()
    }

    /// Declare (once) an exported kernel builtin `symbol` taking `argc`
    /// pointer arguments and returning a pointer, for a direct call.
    fn builtin_fn(&mut self, symbol: &str, argc: usize) -> FunctionValue<'ctx> {
        if let Some(f) = self.module.get_function(symbol) {
            return f;
        }
        let params = vec![self.ptr_t.into(); argc];
        let fn_type = self.ptr_t.fn_type(&params, false);
        self.module
    ...
    fn gen(&mut self, expr: &Expr) -> Option<PointerValue<'ctx>> {
        match expr {
            Expr::Int(n) => {
                Some(self.call_ptr("rt_int", &[self.i64_t.const_int(*n as u64, true).into()]))
            }
            Expr::Float(f) => {
                Some(self.call_ptr("rt_float", &[self.f64_t.const_float(*f).into()]))
            }
            Expr::Chr(c) => Some(self.call_ptr("rt_chr", &[self.i32c(*c as u32).into()])),
            Expr::Str(s) => {
            ...
            Expr::Local(name) => match self.locals[name] {
                ...
                Binding::Slot(slot) => Some(
                    self.builder
                        .build_load(self.ptr_t, slot, name)
                        .unwrap()
                        .into_pointer_value(),
                ),
            },
            Expr::GlobalValue(name) => Some(
                self.builder
                    .build_load(self.ptr_t, self.globals[name].as_pointer_value(), "g")
                    .unwrap()
                    .into_pointer_value(),
            ),
            ...
            Expr::Record(fields) => {
                let record =
                    self.call_ptr("rt_record_new", &[self.i32c(fields.len() as u32).into()]);
            ...
            Expr::Update { record, fields } => {
                ...
                let clone = self.call_ptr("rt_record_clone", &[base.into()]);
            ...
            Expr::Access { record, field } => {
                ...
                Some(self.call_ptr("rt_access", &[r.into(), name_ptr.into()]))
            }
            Expr::GetField { of, step } => {
                let v = self.gen(of)?;
                Some(match step {
                    Step::CtorArg(i) => {
                        self.call_ptr("rt_ctor_arg", &[v.into(), self.i32c(*i).into()])
                    }
                    Step::TupleField(i) => {
                        self.call_ptr("rt_tuple_item", &[v.into(), self.i32c(*i).into()])
                    }
                    Step::ListHead => self.call_ptr("rt_list_head", &[v.into()]),
                    Step::ListTail => self.call_ptr("rt_list_tail", &[v.into()]),
                })
            }
    ...
    fn gen_all(&mut self, exprs: &[Expr]) -> Option<Vec<PointerValue<'ctx>>> {
    ...
    fn finish_merge(
        ...
        Some(
            self.builder
                .build_load(self.ptr_t, slot, "r")
                .unwrap()
                .into_pointer_value(),
        )
    }

    fn gen_test(&mut self, subject: PointerValue<'ctx>, test: &Test) -> IntValue<'ctx> {
        match test {
            Test::IsCtor { index, .. } => {
    ...
    fn prim(&mut self, op: PrimOp, args: &[Expr]) -> Option<PointerValue<'ctx>> {
