...
fn run_both(test_name: &str, source: &str) -> (String, String) {
    let dir = common::test_dir("alm-native-vs-js", test_name);
...
fn chars_and_string_traversal() {
    // Both backends render Chars with single quotes (JS boxes them, native has a
    // distinct Char value), so char Debug.toString agrees with elm and each other.
    assert_same(
        "chars",
        "module Test exposing (..)\n\
         \n\
